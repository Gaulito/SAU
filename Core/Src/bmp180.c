...
// Определяется в соответствии с даташитом
short AC1 = 0;
short AC2 = 0;
short AC3 = 0;
...
uint16_t Get_UTemp (void) // Получение неоткорректированной температуры
{
	uint8_t datatowrite = 0x2E;
	uint8_t Temp_RAW[2] = {0};
	HAL_I2C_Mem_Write(&hi2c1, BMP180_ADDRESS, 0xF4, 1, &datatowrite, 1, 100);
	HAL_Delay (5);  // wait 4.5 ms
	HAL_I2C_Mem_Read(&hi2c1, BMP180_ADDRESS, 0xF6, 1, Temp_RAW, 2, 100);
	return ((Temp_RAW[0]<<8) + Temp_RAW[1]);
}

float bmp180_gettemp (void) // Получение значения температуры
{
	UT = Get_UTemp();
